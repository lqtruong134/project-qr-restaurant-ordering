...
export function buildApp(database: { ping(): Promise<void> }, logging = false) {
  const app = Fastify({ ajv:{customOptions:{removeAdditional:false,coerceTypes:false}}, logger: logging ? {
    redact: ['req.headers.authorization', 'req.headers.cookie', 'res.headers.set-cookie'],
  } : false });
  app.get('/health/live', {config:{public:true}}, async (): Promise<ServiceHealth> => ({ status: 'ok', service: 'api' }));
  app.get('/health/ready', {config:{public:true}}, async (_request, reply): Promise<ServiceHealth> => {
    try {
      await database.ping();
  ...
  app.setErrorHandler((error, request, reply) => {
    if (error && typeof error === 'object' && (('validation' in error && error.validation) || ('statusCode' in error && error.statusCode === 400))) return reply.code(400).send({errorCode:'INVALID_INPUT',userMessage:'Vui lòng kiểm tra thông tin đã nhập.',correlationId:request.id});
