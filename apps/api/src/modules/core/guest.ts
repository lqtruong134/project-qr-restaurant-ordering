...
import { cancelBatch } from './cancellation.js';
...
export function registerGuest(
  ...
  app.post('/guest/join', { config }, async (r, reply) =>
    transaction(db, async (c) => {
      ...
      const table = await one(
        c,
        `SELECT t.* FROM table_qr_token q JOIN dining_table t ON t.id=q.table_id JOIN restaurant r ON r.id=t.restaurant_id WHERE q.token_hash=$1 AND q.status='ACTIVE' AND (q.expires_at IS NULL OR q.expires_at>now()) AND t.restaurant_id=$2 AND t.is_active AND r.status='ACTIVE' FOR UPDATE OF t`,
  ...
  app.patch('/guest/cart/:id', { config }, async (r) =>
    transaction(db, async (c) => {
      ...
      const item = await one(
        c,
        `UPDATE cart_item i SET quantity=$1,note=$2,unit_price_preview=p.base_price FROM product p WHERE i.product_id=p.id AND i.id=$3 AND i.cart_id=$4 AND i.owner_participant_id=$5 AND p.is_active AND p.availability_status='AVAILABLE' RETURNING i.*`,
