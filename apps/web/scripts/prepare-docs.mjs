...
const output=resolve(web,'public/api-docs-assets');mkdirSync(output,{recursive:true});
for(const file of ['swagger-ui-bundle.js','swagger-ui.css'])copyFileSync(require.resolve('swagger-ui-dist/'+file),resolve(output,file));
...
spec.info.description='Đăng nhập bằng API login; trình duyệt tự giữ cookie. Không cần nhập token vào Authorize. Mỗi account có một phiên nội bộ. Các thao tác Execute gọi backend thật.';
const descriptions={
 ...
 '/auth/refresh':['02. Đăng nhập và phiên','Làm mới phiên — xoay vòng refresh cookie','refresh'],
 '/workspaces/{area}':['03. Phân quyền','Kiểm tra quyền vào workspace — 200 hoặc 403','workspace'],
};
for(const [path,operations] of Object.entries(spec.paths))for(const [method,operation] of Object.entries(operations)){
 const [tag,summary,id]=descriptions[path];operation.tags=[tag];operation.summary=summary;operation.operationId=id;
 if(method==='post')for(const parameter of operation.parameters??[])if(parameter.$ref?.endsWith('/Csrf'))parameter.description='Đã điền sẵn 1. Swagger tự gửi header chống CSRF.';
}
spec.components.parameters.Csrf.schema.default='1';
spec.paths['/auth/login'].post.requestBody.content['application/json'].example={username:'staff',password:'THAY_BANG_MAT_KHAU_DEMO_TREN_MAY'};
