...
export async function api<T>(
  ...
  if (!response.ok) throw new Error(data.userMessage ?? 'Không thực hiện được yêu cầu.');
  return data as T;
}
...
export function Action({ children, run }: { children: React.ReactNode; run: () => Promise<void> }) {
  const [busy, setBusy] = useState(false),
    [error, setError] = useState('');
  return (
