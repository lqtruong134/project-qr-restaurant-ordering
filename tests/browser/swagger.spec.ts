...
test('Swagger Execute uses real cookie auth: 401, login, 200, 403, refresh, logout', async ({
  page,
}, info) => {
  ...
  async function execute(id: string, status: number, edit?: () => Promise<void>) {
    ...
    const tryButton = op.getByRole('button', { name: 'Try it out' });
    if (!(await op.getByRole('button', { name: 'Execute', exact: true }).isVisible()))
      await tryButton.click();
    if (edit) await edit();
    await op.getByRole('button', { name: 'Execute', exact: true }).click();
    await expect(
      op.locator('.responses-inner .live-responses-table .response-col_status:not(.col_header)'),
    ).toHaveText(String(status));
  }
  await execute('ready', 200);
  ...
  await execute('workspace', 403, async () => {
    await workspace.locator('select').first().selectOption('admin');
  });
