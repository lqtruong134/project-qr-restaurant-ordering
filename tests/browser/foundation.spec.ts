...
test('foundation page reflects actual API and database readiness', async ({ page, request }, testInfo) => {
  const errors: string[] = [];
  page.on('pageerror', error => errors.push(error.message));
  await expect.poll(async () => (await request.get('http://127.0.0.1:4000/health/ready')).status()).toBe(200);
  await page.goto('/');
