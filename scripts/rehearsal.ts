...
try {
  for (let run = 1; run <= 2; run++) {
    const name = 'thesis_rehearsal_' + randomBytes(6).toString('hex');
    ...
    try {
      ...
      const counts = async () => {
        ...
        for (const { tablename } of rows.rows) {
          ...
          result[tablename] = Number(
            (await db.pool.query('SELECT count(*) AS n FROM ' + tablename)).rows[0].n,
          );
        }
        return result;
      };
      ...
      try {
        for (const role of ['staff', 'kitchen', 'admin']) {
          ...
          for (const area of ['staff', 'kitchen', 'admin'])
            assert.equal(
              (await app.inject({ url: '/workspaces/' + area, headers: { cookie } })).statusCode,
              area === role ? 200 : 403,
            );
        }
      } finally {
...
writeFileSync('docs/evidence/rehearsal.md', logs.join('\n') + '\n');
