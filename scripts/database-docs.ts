...
try {
  const tables = (
    await db.pool.query<{ name: string; description: string }>(
  ...
  for (const table of tables) {
    const cols = (
      await db.pool.query<{
        ...
        `SELECT a.attname name,format_type(a.atttypid,a.atttypmod) type,a.attnotnull required,pg_get_expr(d.adbin,d.adrelid) default_value,col_description(a.attrelid,a.attnum) description FROM pg_attribute a JOIN pg_class c ON c.oid=a.attrelid LEFT JOIN pg_attrdef d ON d.adrelid=a.attrelid AND d.adnum=a.attnum WHERE c.oid=$1::regclass AND a.attnum>0 AND NOT a.attisdropped ORDER BY a.attnum`,
        [table.name],
      )
    ).rows;
    const constraints = (
      await db.pool.query<{ name: string; type: string; definition: string }>(
        `SELECT conname name,contype type,pg_get_constraintdef(oid) definition FROM pg_constraint WHERE conrelid=$1::regclass ORDER BY contype,conname`,
        [table.name],
      )
    ).rows;
    const indexes = (
    ...
    for (const c of cols)
      md.push(
        `| ${c.name} | ${c.type} | ${c.required ? 'Không' : 'Có'} | ${c.default_value ?? '—'} | ${c.description ?? '—'} |`,
      );
    md.push(
      '### PK / FK / Unique / Check',
      '```sql',
        ...
        cols
          .map(
            (c) =>
              ...
              (c.name === 'id'
                ? ' [PK]'
                : c.name.endsWith('_id') && c.name !== 'guest_id'
                  ? ' [FK*]'
                  : '') +
    ...
    const fks = (
      await db.pool.query<{ target: string; definition: string }>(
        `SELECT confrelid::regclass::text target,pg_get_constraintdef(oid) definition FROM pg_constraint WHERE conrelid=$1::regclass AND contype='f'`,
        [table.name],
      )
    ).rows;
    for (const fk of fks)
      dot.push(
        fk.target +
          ...
          table.name +
          ' [label="' +
          (table.name === 'session_cart' ? '1 → 0..1' : '1 → 0..N') +
          '",tooltip="' +
          esc(fk.definition) +
          '"];',
      );
  ...
  execFileSync('dot', [
    '-Tpng',
    '-Gdpi=90',
    'docs/database/physical.dot',
    '-o',
    'docs/database/physical.png',
  ]);
